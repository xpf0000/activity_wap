...
var ActivityListPageModel = {
    ...
    getJoinlist : function(callback)
    {
        ...
        Service.articleGetJoinList(this.id,this.page,this.pagesize,function(data)
        {
            abc.handleData(data,callback);
        });
...
var User = {
    ...
    username:'',
    nickname:'尚未登录',
    headimage:'images/user_face02.jpg',
    msg:'登录',
};
...
function checkLogin()
{
    ...
    {
        mainView.router.loadPage({url:'login.html',modal:true});
        return false;
    }
...
requirejs(['main'], function (main) {
    ...
    require(['vue','store','Service','share','wx','xupload','toast','auislide'], function(v,store,s,share,wx) {
        ...
        $$(document).on('pageInit', function (e) {
            ...
            else if(page.name == 'join_list')
            {
                setTimeout(function(){

                    initJoinListJS(page.query);
        ...
        function initIndexJS()
        {
            ...
            var listVM = new Vue({
                ...
                methods:{
                    ...
                    toInfo:function(i)
                    {
                        var view = Number(listVM.info[i].view)+1;
                        listVM.info[i].view = view+"";

                        mainView.router.loadPage({url:'info.html?id='+listVM.info[i].id});
            ...
            getCategory(function(arr)
            {
                if(arr.length > 4)
                {
                    ...
                    var item = {
                        id : 0,
                        title : '全部',
                        url: 'images/class_icon01.png'
                    };

                    category.info.push(item);
        ...
        function initListJS(query)
        {
            ...
            var vm = new Vue({
                ...
                methods:{
                    ...
                    toInfo:function(i)
                    {
                        var view = Number(vm.list[i].view)+1;
                        vm.list[i].view = view+"";

                        mainView.router.loadPage({url:'info.html?id='+vm.list[i].id});
        ...
        function initInfoJS(id)
        {
                        ...
                        else
                        {
                            Service.usersDelJoin(vm.info.id,User,function(b){
                                ...
                                if(b)
                                {
                                    ...
                                    var n = Number(join.info.r_number)-1;
                                    join.info.r_number = n+"";
                                    vm.info.r_number = n+"";
            ...
            Service.articleGetArticle(id,User.id,function(data)
            {
                ...
                if(info)
                {
                    ...
                    if(obj.headimage == null || obj.headimage == undefined)
                    {
                        obj.headimage = "images/user_face02.jpg";
                    }

                    bottom.type = obj.type;
                    vm.info = obj;
                    join.info.r_number = obj.r_number;
        ...
        function initUserIndexJS()
        {
            ...
            if(!UserVM.uinfo.headimage)
            {
                UserVM.uinfo.headimage = "images/user_face02.jpg";
            }
        ...
        function initUserEditJS()
        {
            ...
            var vm = new Vue({
                el: '#user_edit_info',
                data: {
                    headimg:'images/user_face02.jpg',
                    birthday:'',
                    nickname:'',
                    sex:'0',
                    address:'',
        ...
        function initPublishJS(query)
        {
            ...
            var vm = new Vue({
                ...
                data: {
                    list:[],
                    lastindex:-1,
                    cover:'http://images.ihlpp.cn/fengmian.jpg',
                    coverid:'',
                    isnew:false,
                    isactity:false,
                    title:'',
                ...
                methods:{
                        ...
                        p.upBlock = function(data,err){
                            ...
                            if(err == null)
                            {
                                ...
                                if(code == 0)
                                {
                                    if(vm.isnew)
                                    {
                                        if(vm.list.length == 1 && (vm.list[0].imgid == "" || vm.list[0].imgid == undefined))
                                        {
                                            vm.list = [];
                                        }
                    ...
                    doSubmit:function()
                    {
                        ...
                        $$.each(vm.list,function(index,item){

                            if((item.txt != undefined && item.txt != "") || (item.imgid != undefined && item.imgid != ""))
                            {
                                var obj = item;
                        ...
                        if(tlist.length == 0)
                        {
                            var toast = myApp.toast("请至少选择一张图片或文字", '', {});
                            toast.show();
                            return;
                        }

                        var arrString=JSON.stringify(tlist).replace("点击这里输入照片说明","");
                        myApp.showPreloader("上传中...");
        ...
        function initJoinListJS(query)
        {
            ...
            listObj.getJoinlist(getList);

            $$('#join_list').on('infinite', function () {

                listObj.getJoinlist(getList);

            });

            $$('#join_list').on('scroll', handleInfiniteScroll);
